// @ts-nocheck
import { EduManageConfig } from './types';

class ConfigManager {
  private static instance: ConfigManager;
  private config: EduManageConfig | null = null;
  ...
  public initialize(config: EduManageConfig) {
    if (this.config) {
  ...
  public getConfig(): EduManageConfig {
    if (!this.config) {
  ...
  private createDefaultConfig(): EduManageConfig {
  ...
  public updateConfig(updates: Partial<EduManageConfig>) {
    const config = this.getConfig();
...
export const createConfig = (options: CreateConfigOptions = {}): EduManageConfig => {
  let authConfig;
